...
def fetch_n_map_sensors(sensors,controllers):
    sensor = collection.find_one("sensor_manager")
    meta = requests.get("http://"+sensor["ip"]+":"+sensor["port"]+"/findfreenodes").json()
    # 10.2.131.37:8056
...
def send_controller(controller_name,action,instance=None):
    if instance==None:
        for dev in devices["controllers"][controller_name]:
            requests.post("http://10.2.130.191:9825/actionManagerAPI", json={{"user_id":"{consumer_group.split('_')[0]}","new_value":action,"device_id":dev}})
    else:
        requests.post("http://10.2.130.191:9825/actionManagerAPI", json={{"user_id":"{consumer_group.split('_')[0]}","new_value":action,"device_id":devices["controllers"][controller_name][instance]}})

def send_notif(name, receiver):
    if "email" in notif:
        requests.post("http://10.2.130.191:9825/messageAPI", json={{"number":notif["mobile"], "message":name"hello"}})
    if "sms" in notif:
        requests.post("http://10.2.130.191:9825/emailAPI", json={{"email":receiver, "subject":"Platform Alerts", "text":"hello"}})
        ''')
